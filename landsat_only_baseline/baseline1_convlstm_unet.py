...
class ConvLSTMCell(nn.Module):
    def __init__(self, in_ch: int, hidden_ch: int) -> None:
        ...
        self.conv = nn.Conv2d(in_ch + hidden_ch, hidden_ch * 4, kernel_size=3, padding=1)
...
class ConvLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = torch.zeros(batch, self.cell.hidden_ch, height, width, device=x.device, dtype=x.dtype)
...
class ConvLSTMUNet(nn.Module):
    def __init__(self, hidden_ch: int = 32) -> None:
        super().__init__()
        self.convlstm = ConvLSTM(1, hidden_ch)
        self.enc1 = ConvBlock(hidden_ch, hidden_ch)
        self.pool1 = nn.MaxPool2d(2)
        self.enc2 = ConvBlock(hidden_ch, hidden_ch * 2)
        self.pool2 = nn.MaxPool2d(2)
        self.enc3 = ConvBlock(hidden_ch * 2, hidden_ch * 4)
        self.up2 = nn.ConvTranspose2d(hidden_ch * 4, hidden_ch * 2, kernel_size=2, stride=2)
        self.dec2 = ConvBlock(hidden_ch * 4, hidden_ch * 2)
        self.up1 = nn.ConvTranspose2d(hidden_ch * 2, hidden_ch, kernel_size=2, stride=2)
        self.dec1 = ConvBlock(hidden_ch * 2, hidden_ch)
        self.out = nn.Conv2d(hidden_ch, 1, kernel_size=1)
...
def main() -> None:
    parser = argparse.ArgumentParser("ConvLSTM-UNet baseline")
