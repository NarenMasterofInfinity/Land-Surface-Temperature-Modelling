...
class TemporalAttentionUNet(nn.Module):
    def __init__(self, base_ch: int = 32, num_heads: int = 4) -> None:
        super().__init__()
        self.encoder = Encoder(base_ch=base_ch)
        self.attn = nn.MultiheadAttention(embed_dim=base_ch * 4, num_heads=num_heads, batch_first=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(steps):
            e1, e2, b = self.encoder(x[:, t])
            enc1_list.append(e1)
            enc2_list.append(e2)
            bott_list.append(b)

        enc1 = torch.stack(enc1_list, dim=0).mean(dim=0)
        enc2 = torch.stack(enc2_list, dim=0).mean(dim=0)
        bott = torch.stack(bott_list, dim=1)

        batch, seq_len, ch, h, w = bott.shape
        tokens = bott.permute(0, 3, 4, 1, 2).reshape(batch * h * w, seq_len, ch)
        attn_out, _ = self.attn(tokens, tokens, tokens)
        agg = attn_out.mean(dim=1).reshape(batch, h, w, ch).permute(0, 3, 1, 2)
...
def main() -> None:
    parser = argparse.ArgumentParser("Temporal attention UNet baseline")
    ...
    model_name = "temporal_attention_unet"
    ...
    model = TemporalAttentionUNet().to(device)
