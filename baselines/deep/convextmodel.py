...
OUT_DIR = PROJECT_ROOT / "metrics" / "deep_baselines" / "convext"
FIG_DIR = OUT_DIR / "figures"
MODEL_DIR = PROJECT_ROOT / "models" / "deep_baselines" / "convext"
LOG_DIR = PROJECT_ROOT / "logs" / "new"
LOG_PATH = LOG_DIR / f"convext_train_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def get_mask_channel_indices(comp):
    ...
    # treat world/dyn as categorical -> no normalization
    if "world" in offsets:
...
def compute_input_stats(dataset: TileDataset, items, n_samples, mask_idx):
    ...
    for i in picks:
        ...
        x_raw = np.concatenate([comp[name] for name in INPUT_SOURCES], axis=0)
...
def save_loss_plot(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("loss")
    ax.set_title("convext loss")
    ax.legend()
    fig.savefig(out_dir / "convext_loss.png", dpi=150)
    plt.close(fig)
...
def save_rmse_plot(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("rmse")
    ax.set_title("convext rmse")
    ax.legend()
    fig.savefig(out_dir / "convext_rmse.png", dpi=150)
    plt.close(fig)
...
def save_prediction_figure(y_true: np.ndarray, y_pred: np.ndarray, out_dir: Path) -> None:
    ...
    for ax in axes:
        ax.axis("off")
    fig.savefig(out_dir / "convext_pred_sample.png", dpi=150)
    plt.close(fig)
...
def save_metric_bar(metrics: dict, out_dir: Path) -> None:
    ...
    ax.bar(keys, vals)
    ax.set_title("convext metrics")
    ax.set_ylabel("value")
    fig.savefig(out_dir / "convext_metrics_bar.png", dpi=150)
    plt.close(fig)
...
def save_roi_figure(mask: np.ndarray, out_dir: Path) -> None:
    ...
    ax.axis("off")
    fig.savefig(out_dir / "convext_roi_mask.png", dpi=150)
    plt.close(fig)
...
class ConvExtModel(nn.Module):
    def __init__(self, in_ch: int, width: int = 32, depth: int = 6):
        super().__init__()
...
split_path = OUT_DIR / "convext_date_splits.csv"
...
model = ConvExtModel(in_ch=in_ch).to(device)
...
for epoch in range(1, max_epochs + 1):
    ...
    if np.isfinite(val_loss) and val_loss < best_val:
        ...
        model_path = MODEL_DIR / "convext_best.pt"
        torch.save(
            {
                "epoch": best_epoch,
...
csv_path = OUT_DIR / "convext_metrics.csv"
...
def predict_full_map_convext(model_in, dataset: TileDataset, t_idx: int, device, in_ch: int):
    model_in.eval()
    H, W = H_hr, W_hr
...
def sample_rmse_convext(model_in, dataset: TileDataset, t_idx: int, device, in_ch: int, *, n_tiles: int, seed: int):
    rng = np.random.default_rng(seed)
    errs = []
...
for t in test_dates:
    y_true, y_pred = predict_full_map_convext(model, train_ds, int(t), device, in_ch)
    met = eval_utils.compute_metrics(y_true, y_pred, roi_mask=roi_mask)
    rmse_sampled, n_sampled = sample_rmse_convext(
        model,
        train_ds,
        int(t),
        device,
...
metrics_path = OUT_DIR / "convext_eval_metrics.csv"
