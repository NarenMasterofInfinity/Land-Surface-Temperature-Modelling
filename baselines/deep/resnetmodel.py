...
OUT_DIR = PROJECT_ROOT / "metrics" / "deep_baselines" / "resnet"
FIG_DIR = OUT_DIR / "figures"
MODEL_DIR = PROJECT_ROOT / "models" / "deep_baselines" / "resnet"
LOG_DIR = PROJECT_ROOT / "logs" / "new"
LOG_PATH = LOG_DIR / f"resnet_train_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def save_loss_plot(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.legend()
    fig.savefig(out_dir / "resnet_loss.png", dpi=150)
    plt.close(fig)
...
def save_rmse_plot(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("rmse")
    ax.set_title("resnet rmse")
    ax.legend()
    fig.savefig(out_dir / "resnet_rmse.png", dpi=150)
    plt.close(fig)
...
def save_prediction_figure(y_true: np.ndarray, y_pred: np.ndarray, out_dir: Path) -> None:
    ...
    for ax in axes:
        ax.axis("off")
    fig.savefig(out_dir / "resnet_pred_sample.png", dpi=150)
    plt.close(fig)
...
def save_metric_bar(metrics: dict, out_dir: Path) -> None:
    ...
    ax.bar(keys, vals)
    ax.set_title("resnet metrics")
    ax.set_ylabel("value")
    fig.savefig(out_dir / "resnet_metrics_bar.png", dpi=150)
    plt.close(fig)
...
def save_roi_figure(mask: np.ndarray, out_dir: Path) -> None:
    ...
    ax.axis("off")
    fig.savefig(out_dir / "resnet_roi_mask.png", dpi=150)
    plt.close(fig)
...
split_path = OUT_DIR / "resnet_date_splits.csv"
...
model = ResNetModel(in_ch=in_ch).to(device)
...
for epoch in range(1, max_epochs + 1):
    ...
    if np.isfinite(val_loss) and val_loss < best_val:
        ...
        model_path = MODEL_DIR / "resnet_best.pt"
        torch.save(
            {
                "epoch": best_epoch,
...
csv_path = OUT_DIR / "resnet_metrics.csv"
...
def predict_full_map_resnet(model_in, dataset: TileDataset, t_idx: int, device, in_ch: int):
    model_in.eval()
    H, W = H_hr, W_hr
...
def sample_rmse_resnet(model_in, dataset: TileDataset, t_idx: int, device, in_ch: int, *, n_tiles: int, seed: int):
    rng = np.random.default_rng(seed)
    errs = []
...
for t in test_dates:
    y_true, y_pred = predict_full_map_resnet(model, train_ds, int(t), device, in_ch)
    met = eval_utils.compute_metrics(y_true, y_pred, roi_mask=roi_mask)
    rmse_sampled, n_sampled = sample_rmse_resnet(
        model,
        train_ds,
        int(t),
        device,
...
metrics_path = OUT_DIR / "resnet_eval_metrics.csv"
