...
_ap.add_argument("--run-name", default="cnn_lr_hr", help="Run name for output subdir")
...
logger = logging.getLogger("cnn_lr_hr")
logger.info("starting cnn_lr_hr run=%s", RUN_TAG)
...
def _landsat_date_stats(t_idx):
    ...
    for ys, xs in _iter_chunks(shape, chunks):
        ...
        block_filt, m = _apply_range_mask(block)
    ...
    # median on valid (range-filtered) pixels
...
def save_loss_plot(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("loss")
    ax.set_title("cnn lr/hr loss")
    ax.legend()
    fig.savefig(out_dir / "cnn_lr_hr_loss.png", dpi=150)
    plt.close(fig)
...
def save_rmse_plot(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("rmse")
    ax.set_title("cnn lr/hr rmse")
    ax.legend()
    fig.savefig(out_dir / "cnn_lr_hr_rmse.png", dpi=150)
    plt.close(fig)
...
def save_metric_bar(metrics: dict, out_path: Path) -> None:
    ...
    ax.bar(keys, vals)
    ax.set_title("cnn lr/hr metrics")
    ax.set_ylabel("value")
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
...
split_path = OUT_DIR / "cnn_lr_hr_date_splits.csv"
...
for epoch in range(1, _args.max_epochs + 1):
    model.train()
    train_losses = []
    train_sq = 0.0
    train_n = 0
    ...
    if np.isfinite(val_loss) and val_loss < best_val:
        ...
        model_path = MODEL_DIR / "cnn_lr_hr_best.pt"
        torch.save(
            {
                "epoch": best_epoch,
...
csv_path = OUT_DIR / "cnn_lr_hr_metrics.csv"
...
def predict_full_map(model, dataset: WeakCnnDataset, t_idx: int, device, in_ch: int):
    model.eval()
    H, W = H_hr, W_hr
...
def sample_rmse(model, dataset: WeakCnnDataset, t_idx: int, device, in_ch: int, *, n_tiles: int, seed: int):
    rng = np.random.default_rng(seed)
    errs = []
    kept = 0
...
for t in test_dates:
    ...
    # alignment check samples (10 random patches)
    if align_saved < 10:
        for _ in range(20):
    ...
    # collect patch RMSE for top-20 diagnostics
...
metrics_path = OUT_DIR / "cnn_lr_hr_eval_metrics.csv"
...
if patch_rmse_rows:
    ...
    pd.DataFrame(top).to_csv(OUT_DIR / "cnn_lr_hr_top_patch_rmse.csv", index=False)

if eval_rows:
    first = eval_rows[0]
    save_metric_bar({k: first[k] for k in ("rmse", "ssim", "psnr", "sam", "cc")}, FIG_DIR / "cnn_lr_hr_metrics_bar.png")
