...
DEFAULT_RUN_NAME = "cnn_lr_hr_unet"
MODEL_LABEL = "cnn lr/hr unet"
...
def print_model_summary(model: nn.Module, input_shape: tuple[int, int, int, int]) -> None:
    hooks = []
    rows = []
    ...
    def _hook(name):
        def fn(module, inp, out):
            ...
            rows.append((name, module.__class__.__name__, out_shape, params))
        return fn
    ...
    for name, cls, out_shape, params in rows:
        print(f"{name}: {cls} out={out_shape} params={params}")
...
class ResDown(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, blocks: int = 2):
        ...
        self.act = nn.ReLU()
        self.blocks = nn.Sequential(*[ResBlock(out_ch) for _ in range(blocks)])
...
print_model_summary(model, (1, in_ch, PATCH_SIZE, PATCH_SIZE))
