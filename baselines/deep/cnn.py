...
BASE_OUT_DIR = PROJECT_ROOT / "metrics" / "deep_baselines" / "cnn"
BASE_MODEL_DIR = PROJECT_ROOT / "models" / "deep_baselines" / "cnn"

EPS_Y = 1e-6
INPUT_CHOICES = ["era5", "modis", "viirs", "s1", "s2", "dem", "world", "dyn"]

_ap = argparse.ArgumentParser()
_ap.add_argument(
    "--inputs",
    default="era5,modis,viirs,s1,s2,dem,world,dyn",
    help="Comma-separated input groups",
)
_ap.add_argument(
    "--run-name",
    default=None,
    help="Optional run name for output subdir (defaults to inputs list)",
)
_args = _ap.parse_args()
INPUT_KEYS = [k.strip() for k in _args.inputs.split(",") if k.strip()]
invalid = [k for k in INPUT_KEYS if k not in INPUT_CHOICES]
if invalid:
    raise SystemExit(f"Unknown inputs: {invalid}. Choices: {INPUT_CHOICES}")
RUN_TAG = _args.run_name or "inputs-" + "-".join(INPUT_KEYS)
...
# float maps for bilinear sampling from low-res grids
...
class TileDataset(Dataset):
    ...
    def _build_components(self, t, y0, x0):
        ...
        # target (first landsat band) -> Celsius
    ...
    def __getitem__(self, idx):
        ...
        x = build_input_stack(comp, INPUT_KEYS)
    ...
    def _apply_modis_mask(modis_lr):
        lst, valid, qc = _extract_modis(modis_lr)
        return np.stack([lst, valid, qc], axis=0)

    @staticmethod
    def _apply_viirs_mask(viirs_lr):
        lst, valid, qc = _extract_viirs(viirs_lr)
        return np.stack([lst, valid, qc], axis=0)
...
def sample_values(name, arr, k=5):
    ...
    if finite.size == 0:
        print(f"{name}: all NaN (skipped)")
        return
    vals = finite.ravel()[:k]
    print(f"{name}: {vals.tolist()}")
...
def save_loss_plot(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.legend()
    fig.savefig(out_dir / "cnn_loss.png", dpi=150)
    plt.close(fig)
...
def save_rmse_plot(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("rmse")
    ax.set_title("cnn rmse")
    ax.legend()
    fig.savefig(out_dir / "cnn_rmse.png", dpi=150)
    plt.close(fig)
...
def save_prediction_figure(y_true: np.ndarray, y_pred: np.ndarray, out_dir: Path) -> None:
    ...
    for ax in axes:
        ax.axis("off")
    fig.savefig(out_dir / "cnn_pred_sample.png", dpi=150)
    plt.close(fig)
...
def save_metric_bar(metrics: dict, out_dir: Path) -> None:
    ...
    ax.bar(keys, vals)
    ax.set_title("cnn metrics")
    ax.set_ylabel("value")
    fig.savefig(out_dir / "cnn_metrics_bar.png", dpi=150)
    plt.close(fig)
...
def save_roi_figure(mask: np.ndarray, out_dir: Path) -> None:
    ...
    ax.axis("off")
    fig.savefig(out_dir / "cnn_roi_mask.png", dpi=150)
    plt.close(fig)
...
def predict_full_map_cnn(model, dataset: TileDataset, t_idx: int, device, in_ch: int):
    model.eval()
    H, W = H_hr, W_hr
...
def sample_rmse_cnn(model, dataset: TileDataset, t_idx: int, device, in_ch: int, *, n_tiles: int, seed: int):
    rng = np.random.default_rng(seed)
    errs = []
...
print(f"input_stats computed: ch={in_ch} mask_ch={len(mask_idx)}")

mu_y, sigma_y = compute_target_stats(train_ds, n_samples=200)
print(f"target_stats mu={mu_y:.6f} sigma={sigma_y:.6f}")
...
for epoch in range(1, max_epochs + 1):
    ...
    if np.isfinite(val_loss) and val_loss < best_val:
        ...
        model_path = MODEL_DIR / "cnn_best.pt"
        torch.save(
            {
                "epoch": best_epoch,
    ...
    print(f"epoch={epoch} sample_values (unnormalized)")
    for name in INPUT_KEYS:
        if name in ("modis", "viirs"):
            arr = comp[name]
            lst = arr[0]
            finite = np.isfinite(lst)
            valid_frac = float(np.mean(finite)) if finite.size else 0.0
            lst_mean = float(np.nanmean(lst)) if np.any(finite) else float("nan")
            lst_med = float(np.nanmedian(lst)) if np.any(finite) else float("nan")
            print(f"{name}: lst_mean={lst_mean:.3f} lst_med={lst_med:.3f} valid_frac={valid_frac:.3f}")
            sample_values(f"{name}_lst", lst)
        else:
            sample_values(name, comp[name])
    sample_values("y", comp["y"])
...
csv_path = OUT_DIR / "cnn_metrics.csv"
...
for t in test_dates:
    y_true, y_pred = predict_full_map_cnn(model, train_ds, int(t), device, in_ch)
    met = eval_utils.compute_metrics(y_true, y_pred, roi_mask=roi_mask)
    rmse_sampled, n_sampled = sample_rmse_cnn(
        model,
        train_ds,
        int(t),
        device,
...
metrics_path = OUT_DIR / "cnn_eval_metrics.csv"
