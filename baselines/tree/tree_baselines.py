...
OUT_DIR = PROJECT_ROOT / "metrics" / "tree_baselines"
...
def setup_logging(dataset_key: str, *, start: Optional[str], end: Optional[str]) -> logging.Logger:
    ...
    log_path = LOGS_DIR / f"tree_baselines_{tag}_{range_tag}_{ts}.log"

    logger = logging.getLogger("tree_baselines")
    ...
    logger.info("Tree baselines run start | dataset=%s | start=%s | end=%s | utc=%s",
...
def build_monthly_anchor_cache(
    ...
    *,
    ...
    ds_c = ds_c.chunk({td: 1, "y": 256, "x": 256})
    ...
    def _squeeze_to_xy(da_in: xr.DataArray) -> xr.DataArray:
        extra_dims = [d for d in da_in.dims if d not in (td, "y", "x")]
        for d in extra_dims:
            da_in = da_in.isel({d: 0})
        return da_in

    def _aggregate(month_ds: xr.Dataset, var: Optional[str], masks: List[str]) -> Tuple[Optional[np.ndarray], int]:
        if var is None:
            return None, 0
        data = _squeeze_to_xy(month_ds[var])
        if masks:
            keep = None
            for mv in masks:
                m = _squeeze_to_xy(month_ds[mv])
            ...
            data = xr.where(keep, data, np.nan)
        data = xr.where(np.isfinite(data) & (data > 0), data, np.nan)
        valid_any = data.notnull().any(dim=("y", "x"))
        n_valid_days = int(valid_any.sum().compute().values.item())
        med_map = data.median(dim=td, skipna=True)
        if med_map.size == 0:
            return None, 0
        med_np = np.asarray(med_map.compute(), dtype=np.float32)
        med_np = _mask_lst_values(med_np, var)
        med_np = _convert_lst_units(med_np, var)
        if not np.isfinite(med_np).any():
            return None, 0
        return med_np, n_valid_days

    def _aggregate_qc(month_ds: xr.Dataset, var: Optional[str], masks: List[str]) -> Optional[np.ndarray]:
        if var is None:
            return None
        data = _squeeze_to_xy(month_ds[var])
        if masks:
            ...
            for mv in masks:
                ...
                keep = cond if keep is None else (keep & cond)
            data = xr.where(keep, data, np.nan)
        qc_map = data.median(dim=td, skipna=True)
        if qc_map.size == 0:
            return None
        qc_np = np.asarray(qc_map.compute(), dtype=np.float32)
        if not np.isfinite(qc_np).any():
            return None
        return qc_np
    for t in landsat_times:
        ...
        month_ds = ds_c.isel({td: idx})
        modis_map, n_modis = _aggregate(month_ds, modis_day, modis_masks)
        viirs_map, n_viirs = _aggregate(month_ds, viirs_day, viirs_masks)
        modis_qc_map = _aggregate_qc(month_ds, modis_qc, modis_masks)
        viirs_qc_map = _aggregate_qc(month_ds, viirs_qc, viirs_masks)
...
def sample_xy_for_time(
    ...
    *,
    ...
    for _ in range(spec.max_tiles_per_time):
        ...
        # Anchor features (month-level coarse grids + flags)
        row = (idx // (c1 - c0)) + r0
        col = (idx % (c1 - c0)) + c0
        r_f = row_float[row]
        c_f = col_float[col]
...
def _normalize_pair_for_plot(
    y_true: np.ndarray,
    y_pred: np.ndarray,
) -> Tuple[np.ndarray, np.ndarray, Optional[Tuple[float, float]]]:
    vals = np.concatenate(
        [y_true[np.isfinite(y_true)], y_pred[np.isfinite(y_pred)]]
    )
    if vals.size == 0:
    ...
    # Convert to Celsius-like range for visualization.
    if p98 > 200:
        y_true = y_true - 273.15
        y_pred = y_pred - 273.15
    elif 0.0 <= p2 and p98 <= 1.5:
        y_true = y_true * 50.0
        y_pred = y_pred * 50.0
...
def save_prediction_figures(
    *,
    ...
    y_true, y_pred, pair_vmin_vmax = _normalize_pair_for_plot(y_true, y_pred)
    err = y_pred - y_true
    if vmin_vmax is None:
        if pair_vmin_vmax is not None:
            vmin, vmax = pair_vmin_vmax
        else:
...
def _load_run_config(logger: logging.Logger) -> Dict[str, object]:
    cfg_path = OUT_DIR / "run_config_and_summary.json"
    ...
    try:
        ...
        logger.warning("failed to load run config: %s", exc)
        return {}
...
def _resolve_target_features(
    ...
    if target and features:
        return target, list(features)
    cfg = _load_run_config(logger)
    cfg_target = cfg.get("target") if isinstance(cfg, dict) else None
    cfg_features = cfg.get("features") if isinstance(cfg, dict) else None
    tgt = target or cfg_target or pick_target_var(ds_meta, None)
    if features:
        feats = list(features)
    elif isinstance(cfg_features, list) and cfg_features:
        feats = list(cfg_features)
    else:
        feats = default_feature_vars(ds_meta, tgt, None)
    anchor_names = {
        "modis_cell",
        "modis_up",
        "modis_mean3",
    ...
    feats = [f for f in feats if f not in anchor_names]
    return tgt, feats


def generate_figures_from_saved_models(
    *,
    dataset_key: str,
    figure_date: str,
    target: Optional[str],
    ...
    if td is None:
        raise ValueError(f"{dataset_key} has no time dimension; cannot generate figure.")
    ...
    tgt, feats = _resolve_target_features(ds_meta, target, features, logger)
    fine_shape = (
    ...
    t = pd.Timestamp(figure_date)
    vars_include = [tgt] + [v for v in feats if v != tgt]
    dts = load_subset(
        md,
        dataset_key,
        vars_include=sorted(vars_include),
        start=t,
        end=t,
    )
    if td in dts.dims:
    ...
    if model_name and model_name != "all":
        ...
        model_names = [
            p.stem.replace("tree_", "", 1)
            for p in sorted(MODELS_DIR.glob("tree_*.joblib"))
        ]
    if not model_names:
        raise FileNotFoundError(f"No tree models found in {MODELS_DIR}")

    for name in model_names:
        model_path = MODELS_DIR / f"tree_{name}.joblib"
        if not model_path.exists():
            logger.warning("model=%s missing at %s", name, model_path)
            continue
        import joblib
        model = joblib.load(model_path)
        if getattr(model, "_train_medians", None) is None:
            raise RuntimeError(f"Model {name} missing _train_medians; retrain or update saved model.")
        ...
        fig_paths = save_prediction_figures(
            ...
            y_pred=y_pred,
            model_name=name,
            time_label=str(t.date()),
            roi_mask=roi_mask,
            vmin_vmax=None,
        )
        logger.info("model=%s figure_paths=%s", name, [str(p) for p in fig_paths])
...
def main(
    *,
    ...
    if td is None:
        raise ValueError(f"{dataset_key} has no time dimension; tree baselines need time to split train/test.")
    ...
    vars_keep = [tgt] + [v for v in feats if v != tgt]
    ds = ds[vars_keep]
    y_dim, x_dim = _infer_xy_dims(ds[tgt])
    ds = ds.chunk({td: 1, y_dim: 256, x_dim: 256})
    ...
    sample_n = min(5, X_train.shape[0])
    if sample_n:
        logger.info(
            "sample_X[:%d]=%s",
            sample_n,
            np.array2string(X_train[:sample_n], precision=3, suppress_small=True),
        )
        logger.info(
            "sample_y[:%d]=%s",
            sample_n,
            np.array2string(y_train[:sample_n], precision=3, suppress_small=True),
        )
    ...
    if DecisionTreeRegressor is None:
        logger.warning("DecisionTreeRegressor unavailable; skipping.")
    else:
    ...
    if use_random_forest:
        if RandomForestRegressor is None:
            raise ImportError("scikit-learn RandomForestRegressor unavailable.")
    ...
    if use_xgboost:
        if xgb is None:
            raise ImportError("xgboost unavailable. Install with: pip install xgboost")
    ...
    for name, model in models.items():
        ...
        try:
            ...
            logger.warning("model=%s save failed: %s", name, exc)

        # Feature importance (if available)
        fi = getattr(model, "feature_importances_", None)
        if fi is not None:
            out_fi = OUT_DIR / f"feature_importance_{name}.csv"
            with out_fi.open("w", encoding="utf-8") as f:
                f.write("feature,importance\n")
                for feat, val in sorted(zip(all_features, fi), key=lambda x: x[1], reverse=True):
                    f.write(f"{feat},{float(val)}\n")
            logger.info("model=%s saved_feature_importance=%s", name, out_fi)
        ...
        agg = df[["rmse", "rmse_sum", "ssim", "psnr", "sam", "cc", "ergas"]].mean(numeric_only=True).to_dict()
        agg_std = df[["rmse", "rmse_sum", "ssim", "psnr", "sam", "cc", "ergas"]].std(numeric_only=True).to_dict()
        summary[name] = {"mean": agg, "std": agg_std, "n_test_times": int(len(df))}
        logger.info("model=%s saved_metrics=%s", name, out_csv)
        all_rows.extend(rows)

    pd.DataFrame(all_rows).to_csv(OUT_DIR / "ALL_tree_metrics.csv", index=False)
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser("Tree-based baselines for 30m LST reconstruction")
    ...
    p.add_argument("--use-rf", action="store_true", help="enable RandomForestRegressor")
    p.add_argument("--use-xgb", action="store_true", help="enable XGBoost regressor")
    p.add_argument("--figure-from-saved", action="store_true", help="generate figures from saved models only")
    p.add_argument("--figure-date", default=None, help="date for saved-model figure (YYYY-MM-DD)")
    p.add_argument("--figure-model", default="all", help="saved model name to use (default: all)")
    args = p.parse_args()

    if args.figure_from_saved:
        if not args.figure_date:
            raise SystemExit("--figure-date is required when --figure-from-saved is set")
        generate_figures_from_saved_models(
            dataset_key=args.dataset,
            figure_date=args.figure_date,
            target=args.target,
            features=args.features,
            start=args.start,
            end=args.end,
            model_name=args.figure_model,
