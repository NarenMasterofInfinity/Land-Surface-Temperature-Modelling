...
def _resolve_dataset_paths(
    ...
    *,
    ...
    try:
        resolved = discover_dataset_paths(
            repo_root=repo_root,
            cli_daily=cli_zarr_path,
            cli_30m=cli_zarr_30m_path,
        )
        ds["zarr_path"] = resolved["daily"]
        ds["zarr_30m_path"] = resolved["zarr_30m"]
        return cfg
    except Exception:
        pass

    candidates_daily = [repo_root / "madurai.zarr", Path.cwd() / "madurai.zarr"]
    candidates_30m = [repo_root / "madurai_30m.zarr", Path.cwd() / "madurai_30m.zarr"]
    found_daily = next((p for p in candidates_daily if p.exists()), None)
    ...
    raise RuntimeError(
        ...
        "Use --zarr_path/--zarr_30m_path or set DATASET_ZARR/DATASET_30M_ZARR."
    )
...
def main() -> None:
    ...
    parser.add_argument("--zarr_path", type=str, default=None, help="Override daily zarr path.")
    parser.add_argument("--zarr_30m_path", type=str, default=None, help="Override 30m zarr path.")
    parser.add_argument(
        "--ensemble_runs",
        type=str,
        default=None,
    ...
    for d in np.unique(table.date_idx)[:3]:
        m = table.date_idx == d
        y = table.y[m]
