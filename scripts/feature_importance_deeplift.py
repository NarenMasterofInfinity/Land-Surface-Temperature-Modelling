...
OUT_DIR = Path(_args.out_dir) if _args.out_dir else (PROJECT_ROOT / "figures" / "deep_baselines" / "cnn_lr_hr" / RUN_TAG / "deeplift")
...
class MaskedMeanModel(nn.Module):
    ...
    def forward(self, x):
        ...
        if self._mask is None:
            vals = out.view(out.shape[0], -1).mean(dim=1)
            return vals[:, None]
        mask = self._mask
        if mask.dim() == 2:
            mask = mask[None, ...]
        if mask.shape[0] != out.shape[0]:
            mask = mask.expand(out.shape[0], -1, -1)
        mask_sum = torch.clamp(mask.view(mask.shape[0], -1).sum(dim=1), min=1.0)
        vals = (out * mask).view(out.shape[0], -1).sum(dim=1) / mask_sum
        return vals[:, None]
...
def main():
    ...
    if not pick_idx:
        raise SystemExit("no candidate dates found for DeepLift analysis")
    ...
    print(
        f"[deeplift] candidate_dates={len(date_scores)} "
        f"selected={len(pick_idx)} min_valid_frac={min_date_frac}"
    )
    ...
    masked_model = MaskedMeanModel(model).to(device)
    dl = DeepLift(masked_model)
    ...
    for t in pick_idx:
        ...
        attempts = 0
        print(f"[deeplift] date={date_str} target_samples={_args.samples}")
        while date_count < _args.samples and attempts < _args.samples * 10:
            ...
            if np.mean(v) < 0.3:
                attempts += 1
                if attempts % 5 == 0:
                    print(f"[deeplift] date={date_str} attempts={attempts} samples={date_count} (low valid)")
                continue
            ...
            if not torch.isfinite(mask_t).all() or mask_t.sum().item() == 0:
                attempts += 1
                if attempts % 5 == 0:
                    print(f"[deeplift] date={date_str} attempts={attempts} samples={date_count} (empty mask)")
                continue

            masked_model.set_mask(mask_t)
            attr = dl.attribute(xb, baselines=baseline)
            imp = attr.abs().mean(dim=(0, 2, 3)).detach().cpu().numpy()
            ...
            global_count += 1
            attempts += 1
            print(f"[deeplift] date={date_str} progress={date_count}/{_args.samples} attempts={attempts}")
        ...
        date_csv = OUT_DIR / f"deeplift_channel_importance_{date_str}.csv"
        ...
        ax.set_title(f"DeepLift top channels ({date_str})")
        ax.set_xlabel("importance (mean |attr|)")
        fig.savefig(OUT_DIR / f"deeplift_top20_{date_str}.png", dpi=150)
        plt.close(fig)

        print(f"[deeplift] saved {date_csv}")

    if global_count == 0:
        raise SystemExit("no DeepLift results computed (all dates skipped)")

    global_imp = global_sum / float(global_count)
    global_df = pd.DataFrame({"channel": channel_names, "importance": global_imp})
    global_csv = OUT_DIR / "hrnet_deeplift_channel_importance.csv"
    global_df.to_csv(global_csv, index=False)

    topn = global_df.sort_values("importance", ascending=False).head(20)
    fig, ax = plt.subplots(figsize=(8, 6), constrained_layout=True)
    ax.barh(topn["channel"][::-1], topn["importance"][::-1])
    ax.set_title("DeepLift top channels (overall)")
    ax.set_xlabel("importance (mean |attr|)")
    fig.savefig(OUT_DIR / "hrnet_deeplift_top20.png", dpi=150)
    plt.close(fig)

    print(f"[deeplift] saved {global_csv}")
