...
_ap.add_argument("--checkpoint", default=None, help="Path to hrnet checkpoint (defaults to run best)")
_ap.add_argument("--dates", default=None, help="Comma-separated YYYY-MM-DD dates (2-3 recommended)")
...
OUT_DIR = Path(_args.out_dir) if _args.out_dir else (PROJECT_ROOT / "figures" / "deep_baselines" / "cnn_lr_hr" / RUN_TAG / "inference")
...
def _any_valid_landsat(arr2d):
    arr = _landsat_to_celsius(np.asarray(arr2d))
...
def main():
    ...
    if _args.dates:
        pick_dates = [d.strip() for d in _args.dates.split(",") if d.strip()]
        pick_idx = []
        for d in pick_dates:
            ts = pd.Timestamp(d).normalize()
            m = np.flatnonzero(pd.DatetimeIndex(daily_times).normalize() == ts)
            if m.size:
                pick_idx.append(int(m[0]))
    else:
        landsat_candidates = []
        for t in daily_idx:
            try:
                arr = root_30m["labels_30m"]["landsat"]["data"][int(t), 0, :, :]
                if _any_valid_landsat(arr):
                    landsat_candidates.append(int(t))
            except Exception:
                continue
        pick_idx = landsat_candidates[:3]
    ...
    for t in pick_idx:
        ...
        out_csv = OUT_DIR / "hrnet_inference_metrics.csv"
